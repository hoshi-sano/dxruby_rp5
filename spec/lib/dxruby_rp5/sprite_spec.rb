...
describe DXRubyRP5::Sprite, 'ゲームのキャラを扱う場合の基本となるクラス' do
  ...
  describe '.clean',
    ...
    shared_context '.clean' do
      context 'すべてのvanished?がtrueを返す場合' do
        def make_sprite(i)
          s = double('Sprite')
          allow(s).to receive(:is_a?).and_return(described_class)
...

  {
    ...
    shader: [:shader],
    # Sketch.new される前に実行すると Error になる
    # image: [lambda { DXRubyRP5::Image.load(fixture_path('image.png')) }],
